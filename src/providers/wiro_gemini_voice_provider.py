...
class WiroGeminiVoiceProvider(VoiceProvider):
    """Wiro-based voice provider using Google Gemini 2.5 TTS."""

    def __init__(self, config: ProviderConfig) -> None:
        super().__init__(config)
        self.client = WiroClient(config)
        self.voice = config.voice_id or "Kore"
    ...
    async def generate_speech(self, text: str, output_path: Path) -> tuple[float, str | None]:
        ...
        if not urls:
            raise RuntimeError("No audio output from Wiro Gemini TTS task.")
